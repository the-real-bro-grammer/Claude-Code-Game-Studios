...
> **Status**: REVISED-pending-pass-5-verification (pass-3 /design-review 2026-04-27 returned NEEDS REVISION; pass-4 author-led revision 2026-04-27 applied all 12 BLOCKING + 4 high-priority RECOMMENDED items per pass-3 review log; awaiting pass-5 verification-only re-review in fresh session)
> **Author**: game-designer + unity-specialist + systems-designer + gameplay-programmer + engine-programmer + qa-lead (via /design-system); pass-2 revision via /design-review; pass-3 /design-review (game-designer + systems-designer + qa-lead + unity-specialist + performance-analyst + creative-director); pass-4 author-led revision (no fresh adversarial spawn per pass-3 verification protocol)
> **Last Updated**: 2026-04-27 (pass-4)
> **Last Verified**: 2026-04-27 (pass-3 /design-review)
...
> **Creative Director Review**: pass-1 CD-GDD-ALIGN APPROVED (post-condition-apply) 2026-04-24; pass-2 /design-review NEEDS REVISION → APPLIED 2026-04-27; pass-3 /design-review NEEDS REVISION 2026-04-27 (CD synthesis: "skeleton sound, flesh has hairline fractures; surgical fixes; 2-3 days author-led"); pass-4 author-led revision applied 2026-04-27 — 12 BLOCKING fixes (R10 user-pause preservation, AC-R10b promotion, D.1 per-term overrun, iOS OnApplicationFocus, R7 async contract + R13 sync teardown, AC-EC-E14 split, R9 RequestUnpause guard + memory cycle, R6 UnloadUnusedAssets ordering, R2.b rationale rewrite, R12 destination guard, R14.e InputSystemUIInputModule, third bg_expiry tier) + 4 RECOMMENDED (severity tier count, FEPM CI prereq, AudioListener step 0, AC-R11 10-retry).
...
- **AC-BOOT-FEPM — Fast Enter Play Mode Smoke Test (R2.a) (pass-3 REC-2 — CI prerequisite documented)**. **CI prerequisite (pass-3 REC-2):** before this AC can run, `ProjectSettings/EditorSettings.asset` MUST contain `m_EnterPlayModeOptionsEnabled: 1` AND `m_EnterPlayModeOptions: 2` (which encodes `DisableDomainReload` per Unity 6 EditorSettings serialization — Scene Reload may remain enabled or disabled, both cases must be tested separately). The settings file change MUST be committed and the CI runner must check out a tree containing those settings. Without the prerequisite, the test runs in default-domain-reload mode and provides false-positive coverage (the regression it is designed to catch — static field persistence across cycles — does not even occur). The settings flag is a one-time project-level commit, not a per-test fixture. GIVEN Unity Editor with Fast Enter Play Mode and domain reload disabled (per the prerequisite), WHEN Play is pressed twice in sequence (Play → Stop → Play), THEN the second cycle completes `BootLoader.Awake()` without `InvalidOperationException` or duplicate-singleton warning; all four Foundation siblings reach `Ready` in both cycles. **BLOCKING CI** (new pattern introduced by this GDD)
...
- **AC-R9e — RequestUnpause Step Sequence (R9 RequestUnpause; pass-3 BLOCKING #7 promotion from coverage gap to ADVISORY)**. GIVEN `IsPaused == true` and `PauseOverlay` loaded, WHEN `RequestUnpause()` is called, THEN — verified via sequence-counter log — the steps execute in this order: (1) double-unpause guard checked (`IsPaused == false` → early no-op verified separately by calling `RequestUnpause()` twice in succession); (2) `UnloadSceneAsync("PauseOverlay")` invoked AND awaited to completion; (3) `Resources.UnloadUnusedAssets()` invoked AND awaited (verified by reading the returned `AsyncOperation.isDone == true` after the `await`); (4) ATOMIC `Time.timeScale = 1f` AND `IsPaused = false` (same statement, same step counter); (5) `app.state_changed { isPaused: false }` fires LAST. **ADVISORY (PLAYMODE)** — non-gating; promotes pass-2's "coverage gap" entry to a real test. AC-R9b's BLOCKING tier already covers the pause direction; this is the unpause-direction symmetry.
- **AC-R9f — Pause Overlay Memory Stability Across Cycles (R9 RequestUnpause; pass-3 BLOCKING #11)**. GIVEN a PlayMode test that records `GC.GetTotalMemory(forceFullCollection: true)` baseline AFTER first `RequestPause()` + `RequestUnpause()` cycle (warm-up cycle to amortize first-run allocations), WHEN 10 additional pause/unpause cycles execute back-to-back, THEN `GC.GetTotalMemory(true)` after cycle 11 is within `[baseline - 1MB, baseline + 2MB]` (asymmetric tolerance — a 2MB upward drift across 10 cycles is the failure threshold; the lower bound allows for opportunistic compaction). **PLAYMODE** — measures the BLOCKING #11 fix actually prevents accumulation. Failure mode without `await Resources.UnloadUnusedAssets()` in `RequestUnpause` step 3: typical drift is 5–20 MB across 10 cycles (pause-overlay textures + audio clips not reclaimed). The test runs the warm-up cycle separately to isolate steady-state cycle leak from first-run setup.
...
- **AC-R10b — OS Background + Already Paused: User-Pause Preservation (R10, E9) (pass-3 BLOCKING #2 — promoted ADVISORY → BLOCKING CI (PLAYMODE))**. GIVEN user-paused (`IsPaused == true`) + `OnApplicationPause(true)` fires, WHEN resume fires with elapsed below threshold, THEN: (a) at OS-background time, `_userPausedBeforeBackground = true` is captured (R10 step 2); (b) on resume, `IsPaused = _userPausedBeforeBackground = true` is restored — the user's pause state is preserved; (c) PauseOverlay scene is NOT auto-loaded (the player saw the pause overlay before backgrounding; on resume it is still loaded since the OS-background path did not unload it; if it WAS unloaded, the test asserts the user must explicitly tap pause again — but at MVP the OS-background path does not unload PauseOverlay so this is N/A). The pass-2 ADVISORY tier was wrong because user-pause silent clearance is a P0 contract violation (Section B "seams are where the chaos isn't" fantasy + Pillar 2 player-trust); BLOCKING CI ensures the regression cannot ship. **BLOCKING CI (PLAYMODE)** — uses Unity's `Application.RuntimePlatform` simulation hooks; runs on game-ci/unity-test-runner@v4.
...
### H.3 Edge Cases (8 ACs — pass-3 split AC-EC-E14 → AC-EC-E14a + AC-EC-E14b)
...
- AC-EC-E14a, AC-EC-E14b, AC-EC-E15: Editor-only guards; low regression risk mid-sprint.

### Gate Summary (pass-3 recount — body authoritative; pass-2 reported 46; pass-3 BLOCKING #2 promotes AC-R10b + adds AC-R9e/AC-R9f/AC-EC-E14a/AC-EC-E14b)

| Gate Level | Count | AC IDs | Operability Status |
|------------|-------|--------|--------------------|
| **BLOCKING CI** | 9 | AC-R1, AC-BOOT-FEPM, AC-BOOT-RIOLM, AC-R4, AC-R6, AC-R7a, AC-R9d, AC-INV1, AC-INV2 | **ACTIVE** — AC-R9d requires scene-validator script (Story 2); AC-INV2 condition (d) requires `InputSystemUIInputModule` assertion (pass-3 BLOCKING #12) |
| **BLOCKING CI (PLAYMODE)** | 9 | AC-R2, AC-BOOT-R2, AC-R5, AC-R7, AC-R8, AC-R9a, AC-R9b, AC-R10a, **AC-R10b** | **ACTIVE** at first AppStateManager implementation story; AC-R5 activates once Event Bus absorbs `gameplay.scene.*` channels + `ExclusiveSubscribe` facet (Phase 5 amendment); AC-R10b promoted from ADVISORY in pass-3 BLOCKING #2 (user-pause preservation is a P0 contract) |
| **PLAYMODE** | 16 | AC-R8a, AC-R9c, AC-R9f, AC-R10c, AC-R10d, AC-R12a, AC-R12b, AC-R12c, AC-R13, AC-INV3, AC-D1, AC-EC-E13, AC-EC-E14b, AC-EC-E15, AC-EC-E9, AC-EC-E18 | **PARTIALLY ACTIVE** — most ACTIVE at Stories 1–3; AC-R9f new in pass-3 BLOCKING #11 (memory stability); AC-EC-E14b is the PlayMode side of the pass-3 split |
...
| **ADVISORY** | 3 | AC-D3, AC-EC-E3, AC-R9e | **ACTIVE** at dashboard level — non-gating; AC-R9e new in pass-3 BLOCKING #7 (RequestUnpause sequence; replaces AC-R10b which was promoted out of this tier) |

**Total: 49 ACs** across 9 severity tiers (sum: 9+9+16+2+4+3+2+1+3 = 49). Pass-2 was 46; pass-3 net change is +3 (added AC-R9e, AC-R9f, AC-EC-E14a, AC-EC-E14b; removed AC-EC-E14 via split — the split is +1 net). AC-R10b's tier promotion is not a count change. The pass-3 sum is consistent across H subsections (H.1=30 + H.2=3 + H.3=8 + H.4=2 + H.5=6 = 49) and Gate Summary tier sum (49).
...
- **AC-R12d — `_pendingPause` Destination Permission Guard (pass-3 BLOCKING #10)** — GIVEN `_pendingPause` is queued during a transition that lands in MainMenu/BiomeMap/Shop (any non-Level state), WHEN the transition completes, THEN `_pendingPause` is discarded with an ADVISORY log; no pause-overlay loads; subsequent player input proceeds normally. Add at Story 2 alongside AC-R9c (concurrent-load guard) — both verify the queue's correctness against destination state. **PLAYMODE (pending Story 2 implementation)**.
- **AC-R9e — PROMOTED to ADVISORY in pass-3 BLOCKING #7** — moved into Section H Rule Compliance as a real test (not a coverage gap). Entry retained here for change-log traceability; remove at pass-5.
