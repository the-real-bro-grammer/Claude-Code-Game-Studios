...
> **Status**: REVISED (pass-2 post /design-review 2026-04-27 — 12 BLOCKING items addressed via 5-specialist adversarial pass + creative-director synthesis)
> **Author**: game-designer + unity-specialist + systems-designer + gameplay-programmer + engine-programmer + qa-lead (via /design-system); pass-2 revision via /design-review (game-designer + systems-designer + qa-lead + unity-specialist + performance-analyst + creative-director)
...
> **Creative Director Review**: pass-1 CD-GDD-ALIGN APPROVED (post-condition-apply) 2026-04-24; pass-2 /design-review NEEDS REVISION → APPLIED 2026-04-27 (12 BLOCKING + 13 RECOMMENDED items addressed; 5 convergent specialist findings drove highest-priority fixes — D.1 slack double-count, AC-R5 float equality, missing UnloadUnusedAssets, AC count mismatch, bg_session_expiry default).
...
**R6 — `BeginTransitionAsync` Is the Only Legal Scene-Change Entry Point.** No gameplay or UI code may call `SceneManager.LoadSceneAsync` / `UnloadSceneAsync` directly. The single public method `AppStateManager.BeginTransitionAsync(string targetScene, LoadSceneMode mode)` performs the full sequence: fire `will_unload`, await `Task.WhenAll` of `IPreUnloadHook` handlers (R7), call `LoadSceneAsync` with `allowSceneActivation = false`, **call `Resources.UnloadUnusedAssets()` for the outgoing scene's asset reclaim** (Unity 6 confirms `UnloadSceneAsync` does NOT auto-reclaim — see R11 + D.1 budget), await progress until `>= 0.9f` (NOT exact equality — IEEE 754 float; Unity progress can step 0.85→0.95 in one frame), fire `load_ready`, await activation ack (R8), set `allowSceneActivation = true`, await `sceneLoaded`, update `AppState`, fire `app.state_changed`. Enforcement: CI source-grep rule fails build if any direct `SceneManager.LoadSceneAsync` call exists outside `AppStateManager.cs`.
...
1. If `IsPaused == false` → no-op.
2. `SceneManager.UnloadSceneAsync("PauseOverlay")`; await completion.
3. **ATOMIC: Set `Time.timeScale = 1f` AND `IsPaused = false` together.** Mirror of step 3 above — gameplay resumes only once the overlay is gone.
4. Fire `gameplay.app.state_changed { isPaused: false }`.
...
2. Set `IsPaused = true`.
3. Set `AudioListener.pause = true` (audio mute on OS background; NOT during in-game pause — Audio Bus owns in-game audio pause separately via `gameplay.app.state_changed`).
4. Record `_backgroundedAtUtc = DateTime.UtcNow` AND `_backgroundedDuringRush = (AppState == Level && LevelRuntime.IsInRushPhase)` (the boolean is read via Level Runtime's phase getter; `false` if Level Runtime is not yet authored / scene is not Level).
5. NO overlay load (player isn't looking at screen).
6. NO `gameplay.app.state_changed` fire (Event Bus may already be torn down; subscribers could not execute meaningfully).

When `paused == false` (resume) — **two-tier threshold per pass-2 fantasy correction:**
...
3. Determine threshold: if at background-time the player was in active gameplay (`AppState == Level && Level Runtime is in Rush phase`) → use `bg_expiry_in_rush_seconds` (default **1800s / 30 min**); otherwise (MainMenu, BiomeMap, Shop, Level-but-Prep-phase) → use `bg_expiry_general_seconds` (default **60s**). Rush-phase context is captured at OS-background time via `_backgroundedDuringRush` boolean (set in step 4 of the `paused == true` branch by querying Level Runtime's phase getter).
4. If `elapsed > threshold` → `BeginTransitionAsync("MainMenu")` — treat as fresh session return.
5. Else → set `IsPaused = false`, resume in place. Player sees mid-Rush board exactly as they left it.

**Rationale (pass-2 BLOCK-1 resolution).** The pass-1 single 300s threshold ejected players from mid-Rush after any phone call exceeding 5 minutes — directly violating Section B's "seams are where the chaos isn't" fantasy for the concept doc's target "commuter / break player." The two-tier split protects the highest-stakes case (mid-Rush continuity through typical interruptions: call, text, child interruption, brief errand) while still ejecting quickly from non-gameplay contexts where the in-place resume has no value. Anti-stale safeguard: 30 min upper bound preserves the "session-freshness" fantasy at the long tail.

OS-background path is NOT `RequestPause()`. An OS background while the player was already in `RequestPause()` re-enters the forced-pause path on top; resume restores both states in order (unpause OS first, then user pause remains if it was active).
...
- **Scene Manager: 1100 ms** — scene unload + `Resources.UnloadUnusedAssets()` (mandatory per Unity 6 — `UnloadSceneAsync` does NOT auto-reclaim; `LoadSceneAsync` in `LoadSceneMode.Single` DOES auto-reclaim, but the additive `PauseOverlay` unload path requires manual call) + `LoadSceneAsync` activation + pre-unload hook completion.
...
- `RequestPause()` while `IsPaused == true`: no-op (R9.1). `RequestUnpause()` while `IsPaused == false`: no-op.

**R13 — App-Quit Contract.** `Application.quitting` triggers `AppStateManager.OnApplicationQuit()`, executing the pre-unload hook sequence as R7. **Player builds:** `Task.Wait()` is permitted because no more frames will render. **Editor builds (`#if UNITY_EDITOR`):** `Task.Wait()` is forbidden — domain reload after Editor-quit can interleave with awaited tasks that touch managed Unity objects, throwing `InvalidOperationException` on the stalled domain. In Editor, use a pure-synchronous flush path that does NOT `await` or `Wait()` on any `Task`. Implementation: dispatch via `#if UNITY_EDITOR / #else / #endif` in `OnApplicationQuit()`. `SaveManager.FlushSync()` is required (not `RequestFlush`). `Time.timeScale = 1f`; `AudioListener.pause = false`. After all hooks complete, `AppState = Disposed`. Note: `Application.quitting` is NOT reliable under iOS memory pressure or Android force-kill per engine-programmer §5 — R10's `OnApplicationPause(true)` FlushSync is the real save-safety net on mobile.
...
- **(e)** `EventSystem` lives exclusively in the Boot scene's `DontDestroyOnLoad` hierarchy. No other scene (including `PauseOverlay`) may contain an `EventSystem` component. CI scene-validation script enforces (AC-EC-ES, Section H).
...
| `_backgroundedAtUtc != default` (OS background) | `OnApplicationPause(true)` per R10 | `OnApplicationPause(false)` resume | `FlushSync`; `AudioListener.pause = true`; no overlay; resume threshold check against `bg_session_expiry_seconds` |
...
**Formula (pass-2 reformulated to eliminate slack double-count):**
...
T_slack_consumed = max(0, (T_sm_actual - T_sm_budget) + (T_lr_actual - T_lr_budget))
                                                             (derived diagnostic; NOT additive)
...
- T_sm_actual = 550 ms (Level unload 180 ms + UnloadUnusedAssets 50 ms + LoadSceneAsync 200 ms + handshake 20 ms + pre-unload hooks 100 ms)
...
- T_slack_consumed = max(0, (550-1100) + (380-600)) = max(0, -770) = **0 ms** (both under budget).

**Example (cold Addressables cache, thermal throttle):**
- T_sm_actual = 1050 ms (Level unload 200 ms + UnloadUnusedAssets 70 ms + cold-cache LoadSceneAsync 480 ms + handshake 50 ms + pre-unload hooks 250 ms)
...
- T_slack_consumed = max(0, (1050-1100) + (680-600)) = max(0, -50 + 80) = max(0, 30) = **30 ms** (T_lr 80ms over offset by T_sm 50ms under; net 30 ms of slack used).
...
- T_slack_consumed = max(0, 200 + 250) = **450 ms** (exceeds 300 ms ceiling; analyzer flags both T_sm and T_lr as breaching).
...
### D.3 — OS-Background Session-Expiry Threshold (pass-2 two-tier)

```
elapsed = t_resume - t_background_utc        // clamp to 0 if negative (E12)
threshold = _backgroundedDuringRush
            ? bg_expiry_in_rush_seconds      // default 1800 sec (30 min)
            : bg_expiry_general_seconds      // default 60 sec
if elapsed > threshold:
    BeginTransitionAsync("MainMenu")
else:
    IsPaused = false   // resume in place
```

| Variable | Symbol | Type | Range | Description |
|----------|--------|------|-------|-------------|
| Timestamp at background | t_background_utc | `DateTime` UTC | n/a | Captured at `OnApplicationPause(true)` per R10 step 4. |
| Was in Rush at background? | _backgroundedDuringRush | bool | n/a | Captured at `OnApplicationPause(true)` per R10 step 4 — `true` iff `AppState == Level && LevelRuntime.IsInRushPhase` at that moment. |
...
**Output:** branch decision — `MainMenu` transition vs in-place resume.

**Example (brief phone call mid-Rush, 4 min):**
- `_backgroundedDuringRush = true`; threshold = 1800 sec.
- elapsed = 240 sec < 1800 sec → in-place resume. Player sees their mid-Rush board exactly as they left it. **Section B fantasy preserved.**

**Example (long phone call mid-Rush, 25 min):**
- `_backgroundedDuringRush = true`; threshold = 1800 sec.
- elapsed = 1500 sec < 1800 sec → in-place resume. Mid-Rush state retained even for a typical lunchtime call.
...
- `_backgroundedDuringRush = false`; threshold = 60 sec.
- elapsed = 15 sec < 60 sec → in-place. MainMenu remains visible.
...
**Edge case:** If the system clock was adjusted during the background window (NTP sync, DST), `elapsed` may be negative or spuriously large. If `elapsed < 0` → treat as 0 (in-place resume). If `elapsed > 86400 sec` (24 hr) → clamp to "expired" unconditionally AND trigger the session-continuity integrity check (Save/Load's responsibility, undesigned). The two-tier threshold logic still applies: even with `_backgroundedDuringRush = true`, a 24+ hour absence triggers MainMenu — no Rush state is worth preserving across that gap.
...
**Sums-of-ceilings invariant (pass-2 systems-designer B3):** The Section G tuning knobs allow each per-sibling ceiling to be raised independently to safe-range maxima. Without a constraint, the safe-range maxima sum to 1000+500+1500+800 = **3800 ms** (vs `T_boot_target = 3000 ms`) — meaning a tuner operating within "safe" ranges can collectively exceed the advisory target. Invariant: when raising any `T_x_ceiling` toward its safe-range max, verify `sum(T_x_ceiling) ≤ T_boot_target + 800 ms` (the +800 ms accounts for legitimate margin between nominal worst-case and safe-range max). If the sum would exceed `T_boot_target + 800 ms`, the corresponding lower-ceiling sibling must be tightened in the same change. Add DEVELOPMENT_BUILD log: `[BootLoader] sum(T_x_ceiling) = {sum}ms exceeds T_boot_target + 800ms = {target}; consider tightening` (advisory, non-gating).
...
|------|---------|---------|--------|
| `Assets/Editor/BootEnforcer.cs` | Intercepts Play-from-non-Boot; relaunches via Boot.unity preserving target scene in `EditorPrefs` | E14 + AC-EC-E14 | Pre-implementation — author at first BootLoader story |
...
- **Input System** — add Scene Manager row to Section F Dependencies (currently only references `SceneManager.sceneLoaded` in Section C prose).
- **Event Bus** — add Scene Manager as publisher of `gameplay.scene.*` + `gameplay.app.state_changed` channels in Section C Interactions + Section F Dependencies. Add `IPreUnloadHook` priority 10 registration rule to Event Bus R-rules.
...
The two-tier split protects mid-Rush continuity (the highest-stakes touchpoint per Section B) without retaining stale state for non-gameplay contexts. `_backgroundedDuringRush` boolean is captured at OS-background time per R10 step 4. Anti-stale upper bound on `bg_expiry_in_rush_seconds` preserves the fantasy at the long tail.

Future tuning (post-soft-launch retention data): both knobs are candidates for telemetry-driven adjustment. If retention data reveals players abandoning at the threshold boundary, raise `bg_expiry_in_rush_seconds`; if telemetry shows return-from-MainMenu friction, raise `bg_expiry_general_seconds`. Decision deferred to live-ops.
...
Scene Manager's ACs were validated by `qa-lead` via /design-system Phase 4 mandatory consultation; pass-2 reviewed by `qa-lead` adversarially in /design-review with 6 BLOCKING + 6 RECOMMENDED corrections applied. **46 ACs across 7 severity tiers** (pass-1 reported 36-37 erroneously; pass-2 recount: 28 + 3 + 7 + 2 + 6 = 46), distributed per the project's standard gate pattern (matches Input System pass-6, Event Bus, Save/Load). Coverage prioritizes: R1/R6 grep enforcement, bootstrap ordering (R2/R2.a/R2.b/R3 — including the Fast-Enter-Play-Mode smoke test introduced by R2.a), three-channel event taxonomy (R5 with `>= 0.9f` correction), pre-unload hook contract (R7 with concrete max-vs-sum criterion), activation handshake (R8 with asymmetric tolerance windows), pause lifecycle (R9 with atomic timeScale ordering), OS-background path (R10 + D.3 clock-drift edge cases + two-tier threshold + new AC-EC-E9 mid-transition + new AC-EC-E18 storage-invalidated), retry performance (R11 on ADR-003 device with max-of-5 + monotonic check), re-entrancy (R12 + new AC-R12c firing order), app-quit (R13 with explicit call-after-Disposed verification), invariants (R14.d/e/f), all five formulas (D.1–D.5 with D.1 reformulation), edge cases (E3/E13/E14/E15/E9/E18), and cross-GDD integration with each Foundation sibling.

### H.1 Rule Compliance (28 ACs)
...
- **AC-BOOT-FEPM — Fast Enter Play Mode Smoke Test (R2.a)**. GIVEN Unity Editor with Fast Enter Play Mode and domain reload disabled, WHEN Play is pressed twice in sequence (Play → Stop → Play), THEN the second cycle completes `BootLoader.Awake()` without `InvalidOperationException` or duplicate-singleton warning; all four Foundation siblings reach `Ready` in both cycles. **BLOCKING CI** (new pattern introduced by this GDD)
...
- **AC-R7 — Pre-Unload `Task.WhenAll` + 1000 ms Timeout (R7, D.2) (pass-2 concrete criterion)**. GIVEN three `IPreUnloadHook` handlers at priorities 10/20/30 with mock task durations 50ms / 30ms / 70ms (introduced via `Task.Delay`), WHEN `BeginTransitionAsync` runs, THEN: (a) all three handlers are invoked (verified by sequence counters incremented in each handler); (b) the wall-clock elapsed from first-handler-invoke to all-handlers-complete is `≤ max(50, 30, 70) + 50ms = 120ms` (max-with-tolerance); (c) the elapsed is `< sum(50, 30, 70) - 20ms = 130ms` — proves serialization did not occur (the test fails if elapsed is in [130, ∞)); (d) a never-completing handler test fixture verifies cancellation fires at 1000ms ± 100ms with BLOCKING log in DEVELOPMENT_BUILD; Scene Manager proceeds with unload. **BLOCKING CI (PLAYMODE)** — note: AC-D2 (EditMode) provides the cleaner max-vs-sum proof in pure C#; AC-R7's role is to verify the integration in a Unity scene context.
...
- **AC-R10b — OS Background + Already Paused (R10, E9)**. GIVEN user-paused (`IsPaused == true`) + `OnApplicationPause(true)` fires, WHEN resume fires with elapsed below threshold, THEN `IsPaused` is restored to `true` (user-pause preserved); the user's pause state is not silently cleared by resume. **PLAYMODE (ADVISORY — OS emulation is PlayMode-only)**
- **AC-R10c — Session Expiry Above Threshold (R10, D.3)**. GIVEN `_backgroundedAtUtc` was recorded and elapsed > `bg_session_expiry_seconds`, WHEN `OnApplicationPause(false)` fires, THEN `BeginTransitionAsync("MainMenu")` is called; `IsPaused` is NOT set to false in place. **PLAYMODE**
- **AC-R10d — Session Expiry Below Threshold (R10, D.3)**. GIVEN elapsed < `bg_session_expiry_seconds`, WHEN resume fires, THEN in-place resume: `AudioListener.pause = false`; `IsPaused = false`; no `BeginTransitionAsync`. **PLAYMODE**
...
- **AC-R11 / AC-P-Retry — Retry Performance on Reference Device (R11, D.1) (pass-2 statistics fix + thermal monotonic check)**. GIVEN the ADR-003 reference Android device (Mali-G52 class) in steady-state (warmed up by 60 sec of Level gameplay; thermal junction temp logged), WHEN five consecutive Level → Level retries are measured via wall-clock instrumentation, THEN: (a) **max** of all five ≤ 2000 ms (replaces pass-1 "p95 across 5 samples" which is statistically `max` for n=5); (b) target **mean ≤ 1500 ms** with 500 ms headroom; (c) **monotonic-increase advisory check**: if any retry-N+1 exceeds retry-N by more than 300 ms, log a thermal-degradation ADVISORY warning (does not fail the gate but flags for performance regression dashboard). `T_sm_actual` for each retry is reported individually; `T_slack_consumed` per retry is logged. **DEVICE** (blocked on ADR-003) — for stronger statistical confidence, run a 20-retry variant during pre-launch perf soak.
...
- **AC-INV2 — EventSystem Single-Instance (R14.e)**. GIVEN any non-Boot scene (MainMenu, BiomeMap, Level, Shop, PauseOverlay), WHEN a CI scene-validation script scans each scene file for `EventSystem` components, THEN zero are found; all failing scenes block the build. **BLOCKING CI** (build-time scene validator)
...
- **AC-D1 — Retry Budget Per-Line-Item Attribution (D.1)**. GIVEN a DEVELOPMENT_BUILD, WHEN a Level → Level retry completes, THEN the budget analyzer logs at minimum four per-line-item entries: scene-unload duration, `LoadSceneAsync` duration, activation-handshake duration, pre-unload-hooks duration; each entry includes its value in ms; total `T_sm` equals their sum; `T_sm ≤ 1100 ms` assertion is logged. **PLAYMODE**
...
### H.3 Edge Cases (7 ACs — pass-2 added AC-EC-E9, AC-EC-E18, AC-R12c)
...
- **AC-EC-E14 — Non-Boot-Scene Editor Play: BootEnforcer (E14) (pass-2 rewrite — FoundationStub discarded)**. GIVEN a developer presses Play from `Level.unity` directly (not `Boot.unity`), WHEN the `[InitializeOnLoad]` `BootEnforcer` script's `EditorApplication.playModeStateChanged` callback intercepts the `EnteredPlayMode` transition, THEN: (a) the target scene name `"Level"` is stored in `EditorPrefs` key `MM.Editor.PostBootJumpTo`; (b) Unity loads `Boot.unity` BEFORE Play mode fully enters; (c) `BootLoader.Awake()` runs the production R2.a → R2 init chain on real Foundation siblings; (d) after `BootLoader` completes the init chain, it reads `MM.Editor.PostBootJumpTo`, clears the pref, and calls `BeginTransitionAsync("Level")` instead of the default `"MainMenu"` transition; (e) the developer ends up in `Level.unity` with full production Foundation state, identical to a production cold-start launch. Verified via Editor-only PlayMode test (open `Level.unity`, programmatically trigger Play, assert end state matches Boot-launched-then-transitioned-to-Level). The pass-1 `FoundationStub` prefab approach is **DISCARDED** to eliminate the production/development divergence footgun (see E14 + game-designer REC-4 + qa-lead R4). **PLAYMODE** (Editor-only)
...
- AC-BOOT-R2: sequence-log stub for all four Foundation `Initialize()` methods; `IPreUnloadHook` stub implementations for priorities 10/20/30.
- AC-R5: event-capture fixture that records channel name, payload type, and wall-clock timestamp for all three `gameplay.scene.*` channels; required before the first `BeginTransitionAsync` implementation is merged.
- AC-BOOT-FEPM: the Play-twice CI smoke test fixture must ship alongside the `Reset()` implementation — deferring means FEPM regressions survive until QA hand-off, which is exactly what R2.a was written to prevent.
- AC-R10e + AC-D2: pure C# fixtures (< 30 min each), no Unity scene required; ship at Story 1.

**Scaffold at Story 2 (Pause overlay + OS background):**
- AC-R9a/b: `RequestPause()` sequence-log fixture with timestamped `sceneLoaded` vs `timeScale` write.
- AC-R10a: `OnApplicationPause` 6-condition assertion fixture using mock `ISaveManager`.

**Deferrable to QA hand-off:**
- AC-R12a, AC-INT-EB, AC-INT-SL, AC-INT-Input: post-integration-sprint.
- AC-EC-E14, AC-EC-E15: Editor-only guards; low regression risk mid-sprint.

### Gate Summary (pass-2 recount — body authoritative; pass-1 reported 36-37 erroneously)

| Gate Level | Count | AC IDs | Operability Status |
|------------|-------|--------|--------------------|
| **BLOCKING CI** | 9 | AC-R1, AC-BOOT-FEPM, AC-BOOT-RIOLM, AC-R4, AC-R6, AC-R7a, AC-R9d, AC-INV1, AC-INV2 | **ACTIVE** — AC-R9d requires scene-validator script (Story 2) |
| **BLOCKING CI (PLAYMODE)** | 8 | AC-R2, AC-BOOT-R2, AC-R5, AC-R7, AC-R8, AC-R9a, AC-R9b, AC-R10a | **ACTIVE** at first AppStateManager implementation story; AC-R5 activates once Event Bus absorbs `gameplay.scene.*` channels + `ExclusiveSubscribe` facet (Phase 5 amendment) |
| **PLAYMODE** | 15 | AC-R8a, AC-R9c, AC-R10c, AC-R10d, AC-R12a, AC-R12b, AC-R12c, AC-R13, AC-INV3, AC-D1, AC-EC-E13, AC-EC-E14, AC-EC-E15, AC-EC-E9, AC-EC-E18 | **PARTIALLY ACTIVE** — most ACTIVE at Stories 1–3 |
| **PLAYMODE (DEFERRED)** | 2 | AC-INT-Level, AC-INT-Modal | **DEFERRED** — activates when Level Runtime / Modal/Dialog GDDs land |
...
| **EDITMODE** | 2 | AC-R10e, AC-D2 | **ACTIVE** (pure C# logic, zero external dependency) |
| **DEVICE** | 2 | AC-R11 / AC-P-Retry, AC-P-Boot | **BLOCKED on ADR-003** — activate at Plumbing Sprint / VS milestone |
| **DEVICE (ADVISORY)** | 1 | AC-P-UnityBoot | **BLOCKED on ADR-003** — dashboard only when activated |
| **ADVISORY** | 3 | AC-D3, AC-EC-E3, AC-R10b (dev-build-only log tier) | **ACTIVE** at dashboard level — non-gating |

**Total: 46 ACs** across 9 severity tiers (sum: 9+8+15+2+4+2+2+1+3 = 46). Pass-1 reported 36-37; recount per pass-2 qa-lead B1 finding revealed body had 42 ACs at pass-1; pass-2 revisions added 4 net new ACs (AC-EC-E9, AC-EC-E18, AC-R12c, AC-P-UnityBoot) bringing total to 46. The pass-2 sum is now consistent across header (46), body enumeration (28+3+7+2+6=46), and Gate Summary tier sum (46).
...
- **AC-R9e** (RequestUnpause full sequence: `Time.timeScale` restore, overlay unload, `IsPaused = false` ordering) — complement to AC-R9b; add at Story 2 if needed.
...
### Knowledge gaps — Unity 6.3 API verification (1 open + 6 RESOLVED in pass-2)

Pass-2 unity-specialist verified 6 of the 7 pass-1 OQs against Unity 6.0 official docs (`https://docs.unity3d.com/6000.0/...`). Six are **CLOSED**; OQ-1 remains open pending knowledge gap.

| OQ | Topic | Status | Resolution |
|----|-------|--------|-----------|
| **OQ-1** | Does `LoadSceneParameters` (Unity 2019+) have new flags in Unity 6.x? | **OPEN** | Defer to first implementation story; Unity 6.x docs review needed for `LoadSceneParameters` struct field changes. Low risk — R1 assumes standard `LoadSceneMode` enum which is unchanged. |
...
**Total: 20 OQs** — 1 open Unity API + 6 RESOLVED Unity API + 5 cross-GDD provisional + 2 deferred design + 1 ADR-003 shared + 5 NEW pass-2 (4 cross-GDD/architecture, 1 live-ops, 1 device-measurement). Net active: 13 (excluding 6 RESOLVED + 1 deferred-low-risk).
