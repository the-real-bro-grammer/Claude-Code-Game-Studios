...
> **Status**: Revised three times after `/design-review` — pending targeted pass-4 verification of Tier-1 bundles (per creative-director's pass-3 recommendation) in a fresh session
...
>
> **/design-review pass 3 (2026-04-21)**: NEEDS REVISION → RESOLVED in this revision pass. Independent fresh-session re-review by 6 specialists + creative-director synthesis surfaced a third stratum of 21 blocking items (6 Tier-1 convergent bundles + 2 Tier-2 unified fixes + 13 surgical edits) — primarily downstream-system implications rather than GDD self-consistency defects. Pass-3 fix session applied in a fresh session with targeted checklist (per creative-director's recommendation to avoid pattern-blindness of same-session revision). User adjudicated the performance-analyst-vs-creative-director disagreement on Mali-G52 scaling in favor of the hard-deadline split (AC-P2a sprint ADVISORY + AC-P2b VS BLOCKING). Fixes applied: **Tier 1** — D.3 V_i excludes gems until Currency & Economy GDD defines coin-per-gem rate (with OnValidate warning on gem-bearing LootTables referenced by LevelData); StarThresholdConfig denominator anchored to `LevelData.quota` (not Ceiling) in Category #8; ProgressionNode unlock condition-type enum closed to `{LEVEL_COMPLETE, STAR_COUNT, COIN_TOTAL, TOOL_OWNED}` gated by Monetization ADR; ToolConfig upgrade cost monotonicity enforced in Category #1 + new AC-R5d BLOCKING CI; AC-R8e spread ceiling added (no loadout > 40pp 3-star rate above worst loadout) + 60% floor and 40% ceiling both registered as tuning knobs; AC-NHV non-empty-bypass guard (`wc -l < 7` fails CI). **Tier 2** — new R4.a Asset Import Protocol subsection specifying StartAssetEditing/StopAssetEditing bracketing + empty-GUID sentinel guard + explicit delayCall drain pattern; Gate Summary recounted to 42 AC identifiers / 26 scaffolding units (pass-2 count was self-inconsistent at 39/28 and 38/28). **13 surgical edits** — EC-R2 requires Domain Reload enabled (fast-enter serves stale Dictionary); R5 Layer 1 specifies `Debug.LogWarning(message, this)` two-arg form; AC-R4 specifies callbackOrder ≥ 100 and adds explicit BuildReport assertion; new AC-R7a event-ID constant ↔ taxonomy drift test; AC-P1 std-dev clamp (< 20% of median, extends to 30 iterations if needed); AC-P4 per-invocation scope with delta-only check requirement; AC-R5b fixture-existence `[OneTimeSetUp]` precheck; AC-EC-A2 split into MANUAL Part 1 (rename+move evidence artifact) + BLOCKING CI Part 2 (automated GUID consistency); ProgressionNode rewardAmount bounded (COINS ∈ [1, 10_000]; GEMS ∈ [1, 500]); E_norm denominator parameters required strictly positive in Category #1; CurrencyDefinition earn-rate unit locked as `earnRateBaselineCoinsPerCompletedLevel`; AC-R8d strict `>` pinning at exactly 0.30f and 31/100. Awaiting targeted pass-4 verification of Tier-1 bundles (not a full re-review).
>
> **/design-review pass 2 (2026-04-21)**: NEEDS REVISION → RESOLVED in that revision pass. Independent re-review by 6 specialists + creative-director synthesis found 12 new blocking items in the revision pass 1 output, all addressed here: (1) R3.a added — Catalog population via AssetPostprocessor auto-scan (was unspecified, day-one implementer blocker); (2) R4 rewritten for Unity 6.3 API currency (`AssetDatabase.GUIDFromAssetPath(path).ToString()` replaces deprecated `AssetPathToGUID`) + split into Mechanism A (OnValidate always-check) + Mechanism B (AssetPostprocessor move-path, because OnValidate does not reliably fire on folder-move); (3) R4 Persistence pattern added — `EditorUtility.SetDirty(this)` via `EditorApplication.delayCall` to flush `_id` writes to disk (previously `_id` lived in memory only, CI could pass on-disk stale); (4) EC-A1, EC-A2, EC-B2 updated to match the new R4 mechanisms and reference on-disk fixture paths; (5) AC-R4 rewritten to force `ImportAsset(ForceUpdate)` before assertion + wired into `IPreprocessBuildWithReport` for pre-build verification; (6) AC-EC-B2 uses an on-disk fixture pair at `tests/editmode/data-registry/fixtures/hex-edit/` (was unimplementable "simulated" test); (7) AC-R5b fixture existence note added, naming `tests/editmode/data-registry/fixtures/layer2/` as a pre-committed artifact; (8) AC-R1b rewritten as a Roslyn analyzer AC (grep cannot resolve receiver types); (9) AC-D1 fixture swapped from `[0.01, 99.99, 0.01]` to `[1, 9999, 1]` (float32-safe) + normalization computation specified in `double`; (10) AC-D2 full 3-tool synthetic category fixture spelled out (was "synthetic category whose mean is 0.05" with no peer definition); (11) AC-P1 rewritten to measure first-`GetById` call, not `OnEnable` (under EC-R2 lazy-init, `OnEnable` builds nothing); per-Catalog budget reduced to 4ms so 11 × 4 = 44ms fits AC-P2's 50ms aggregate with 6ms headroom; (12) AC-P2 escape hatch replaced with a concrete ticket requirement + cold/warm-start clarification; Gate Summary recounted (19 BLOCKING CI + 6 ADVISORY + 11 DEFERRED + 3 MANUAL = 39 AC identifiers; EC-F1 folded into AC-D3's parameter array as a fixture row, not a separate test file); ProgressionNode reward type enumerated to a closed MVP allowlist (`{COINS, GEMS, TOOL_UNLOCK, LEVEL_UNLOCK, BIOME_UNLOCK}`) gated by Monetization ADR for future extensions. Remaining deferred RECOMMENDED items (not blocking): E_norm signal-limits disclaim, AC-R8e equivalence clause + tuning-knob registration, `_excludeFromQuotaSimulation` reference-level migration, StarThresholdConfig denominator anchor (hand to Level Scoring GDD), ToolConfig monotonic-cost validation, regex digits decision — see `design/gdd/reviews/data-registry-review-log.md` pass 2 entry.
...
**R4.a Asset Import Protocol.** The R4 mechanisms and R5 Layer 2 postprocessor share a common editor-time import pipeline that MUST observe three protocol rules to stay correct under mass-import events (git pull, large asset drops, batch renames):

- **Batch bracketing.** Any bulk write path that touches more than one asset (Mechanism B iterating `movedAssets[]`, R3.a postprocessor iterating `importedAssets[]`, or a test harness that creates many fixtures) MUST bracket its writes with `AssetDatabase.StartAssetEditing()` / `AssetDatabase.StopAssetEditing()`. Without this, each per-asset `SetDirty` + `SaveAssetIfDirty` queued via `EditorApplication.delayCall` accumulates a separate import pass, and large batches can recurse into the postprocessor before the first pass completes.
- **Empty-GUID guard.** `AssetDatabase.GUIDFromAssetPath(path).ToString()` returns the empty-GUID sentinel string `"00000000000000000000000000000000"` for assets that Unity has not yet assigned a GUID to (notably brand-new assets mid-import, before the `.meta` file has been written). Every `_id` write path (Mechanism A, Mechanism B, R3.a postprocessor) MUST check for this sentinel and skip the write when it is detected; the next `OnValidate` or next import pass re-runs with a real GUID available and writes correctly.
- **Explicit flush in tests.** `AssetDatabase.ImportAsset(path, ImportAssetOptions.ForceUpdate)` does NOT flush `EditorApplication.delayCall`-deferred `SetDirty` queues (the callbacks run on the next editor tick, not during `ImportAsset`). AC-R4 and any test that asserts on-disk `_id` state MUST invoke `EditorApplication.delayCall.Invoke()` (or `yield return null` in a coroutine test context) to drain the delayCall queue before the on-disk assertion, then call `AssetDatabase.SaveAssetIfDirty(asset)` explicitly.

These three rules are normative for any programmer implementing R4, R3.a, or R5 Layer 2.
- **Layer 1 — `OnValidate()`** per SO class: catches out-of-range values, null required references, empty display names. Logs via the two-argument overload `Debug.LogWarning(message, this)` so the Console entry is clickable and selects the offending asset in the Project view (the single-arg `Debug.LogWarning(this)` form silently stringifies `this` as the message and produces a non-clickable entry) — never throws.
...
|---|---|---|---|---|
| 1 | `ToolConfig` | Type, cost, effect params, upgrade tier chain | Tool System | `C_base > 0`; upgrade chain acyclic; all tier refs resolve. **Upgrade costs strictly monotonic non-decreasing** (`cost[i] ≤ cost[i+1]` across `upgradeTierChain`) — the MVP shop reads this chain directly so a non-monotonic entry is a live shopping-bug (see AC-R5d). **E_norm denominator parameters strictly positive**: Ramps `redirectAngleRangeDegrees > 0`; Nets `catchZoneAreaTiles > 0`; Guard Posts `coverageRadiusTiles > 0` — closes the degenerate `E_norm = 0` path that would silently depress `CategoryMean_k` and mask real dominance |
...
| 7 | `CurrencyDefinition` | Currency ID (coin/gem), name, icon, `earnRateBaselineCoinsPerCompletedLevel` (MVP unit — see note), IAP product ID (stub at MVP) | Currency & Economy | `earnRateBaselineCoinsPerCompletedLevel > 0` for the coin currency (MVP unit = coins earned per average 2-star level completion; used as the denominator in Currency & Economy balance audits); gem currency may set the field to `0` since gems are not earned from level completion at MVP; icon non-null; no duplicate IDs. **Unit is locked at MVP**; unit changes require a Currency & Economy GDD update because downstream systems compare against this baseline |
| 8 | `StarThresholdConfig` | Level ref, 1/2/3-star quota floors | Level Scoring | 0 < 1-star < 2-star < 3-star ≤ 1.0. **Thresholds are ratios of `LevelData.quota`, not of `QuotaAchievabilityCeiling`** — a 3-star threshold of 1.0 means "banked ≥ quota," not "banked ≥ ceiling"; this anchor is load-bearing and must not drift when Level Scoring GDD lands (forward pointer: see `design/gdd/level-scoring.md` when authored) |
| 9 | `ProgressionNode` | Node ID, unlock **condition type** (enum: `LEVEL_COMPLETE` / `STAR_COUNT` / `COIN_TOTAL` / `TOOL_OWNED`) + threshold, reward **type** (enum: `COINS` / `GEMS` / `TOOL_UNLOCK` / `LEVEL_UNLOCK` / `BIOME_UNLOCK`) + ref | Progression | Unlock condition type must be one of the four MVP-allowlisted enum values (R5 rejects unlisted values at `OnValidate`); reward type must be one of the five MVP-allowlisted enum values; reward ref resolves (ref type matches reward type — `TOOL_UNLOCK` refs `ToolConfig`, `LEVEL_UNLOCK` refs `LevelData`, `BIOME_UNLOCK` refs `BiomeData`; `COINS` / `GEMS` carry no ref and an integer `rewardAmount` field — **COINS ∈ [1, 10_000]; GEMS ∈ [1, 500]** upper bounds enforced at `OnValidate` so an unbounded `int.MaxValue` reward cannot slip through); no circular deps; threshold > 0. **Both enums are closed per anti-pillar "no pay-to-win" — new reward types (e.g., `COSMETIC_UNLOCK`, `IAP_UNLOCK`) and new condition types (e.g., `IAP_PURCHASED`, `WATCH_AD`) require a Monetization Boundary ADR entry before the enum may be extended.** The condition-type enum is the pay-to-win unlock-vector guard; leaving it open would re-introduce the same hazard pass 2 closed on the reward side. |
...
| Expected loot value per arc | EV_loot | float | [0.5, 50.0] coins | `Σ (P_i * V_i)` across referenced LootTable entries, where `V_i` is the **coin-denominated** mid-point value of entry i's qty range. **Gem entries are excluded from `EV_loot`** at MVP — project-wide coin-per-gem conversion is intentionally undefined until the Currency & Economy GDD establishes exchange semantics, and a numeric guess here would silently bias every quota achievability audit. A LootTable referenced by any `LevelData` that contains at least one gem-currency entry triggers an `OnValidate` WARNING on the `LevelData` explicitly naming that `EV_loot` is computed on coin yield only; the LootTable author may either (a) leave the gem entry in and accept that quota math measures only the coin path, or (b) set `_excludeFromQuotaSimulation = true` per EC-F4 if gems are the dominant yield. When Currency & Economy GDD lands at VS, this field promotes to include a conversion term (target: `V_i_coins + (V_i_gems * gemToCoinRate)`); the conversion rate lives in `CurrencyDefinition` as a new field gated by a Monetization Boundary ADR entry |
...
- **EC-R2 — If `Catalog.GetById(id)` is called before `OnEnable` completes (race during domain reload)**: the internal `Dictionary` is not yet built; `GetById` returns null. Mitigation: `Catalog` uses **lazy initialization** — the `Dictionary` is built on the first `GetById` or `All` call if `OnEnable` has not already populated it, guarded by a null-check on the dictionary field. This eliminates the race entirely without requiring execution-order control. Note: `[DefaultExecutionOrder]` is a MonoBehaviour attribute and does NOT apply to ScriptableObjects; the lazy-init pattern is the only viable race mitigation for SO-based catalogs. Consumers should still cache resolved SO references at `Awake`/`Start` rather than calling `GetById` per frame (see hot-path rule in Tuning Knobs). **Domain Reload required (DoD constraint).** Unity 6.3's "Enter Play Mode Options → Reload Domain = Disabled" fast-enter mode preserves static state across PlayMode entries, including the already-built lazy-init `Dictionary` from a prior session; stale entries from before an asset rename/move/delete would silently be served. Data Registry requires **Reload Domain = Enabled** on every developer machine and in CI; the Data Registry implementation sprint Definition of Done MUST include a checklist item verifying `ProjectSettings/EditorSettings.asset → EnterPlayModeOptionsEnabled = 0` (or `EnterPlayModeOptions` omitted entirely). Disabling fast-enter is a non-negotiable correctness constraint, not a performance preference.
...
| AC-R8e cross-loadout viability floor (Pillar 3 VS playtest) | 0.60 (each of the ≥ 2 loadouts must 1-star ≥ 60% of attempted levels) | [0.50, 0.75] | More permissive — loadouts with ≥ 50% pass rate count as "viable"; risk: low-floor loadouts ship and Pillar 3 drifts | Stricter — loadouts below 75% pass rate fail the gate; risk: legitimately specialized loadouts (e.g., tempo-heavy) get rejected for context-dependent weakness |
| AC-R8e cross-loadout spread ceiling (Pillar 3 VS playtest) | 0.40 (no loadout may 3-star at a rate more than 40 percentage points above the worst loadout on the same level set) | [0.30, 0.50] | More permissive — wider spread allowed before Pillar 3 fails; risk: a loadout 3-stars 80% while another 3-stars 35% (45pp gap) passes, which is the "participation trophy" failure mode pass 3 flagged | Stricter — tighter spread enforced; risk: legitimately tiered loadout identities (boss-focused vs. volume-focused) can't both ship when one is naturally higher-ceiling. **Interaction with the floor**: 60% floor alone passes a 2.8★-vs-1.1★ pair; 40% spread ceiling alone passes any two uniformly-weak loadouts. Both gates must pass together for Pillar 3 validation |
...
After pass-3 revision: **42 distinct AC identifiers** across 6 categories (authored ACs grew by +3 vs. pass 2: new AC-R5d monotonic upgrade cost, new AC-R7a event-ID drift, AC-P2 split into P2a/P2b; EC-F1 remains folded into AC-D3 as a non-standalone gate). See the Gate Summary table at the end of this section for the full count breakdown. Gate classifications: **BLOCKING CI** (must pass for merge) / **ADVISORY** (logged, non-blocking) / **PLAYMODE** (requires Unity PlayMode test runner) / **MANUAL** (designer-visible check or playtest artifact) / **BLOCKING at VS gate** (deferred from this sprint to Vertical Slice). Consumer integration ACs (C1-C11) are DEFERRED per-system and are not gates on the Data Registry sprint itself.
...
- **AC-R4 — GUID Stability** — GIVEN any SO asset in the registry, WHEN inspected in the Editor or via EditMode test, THEN its serialized on-disk `_id` field equals `AssetDatabase.GUIDFromAssetPath(assetPath).ToString()` (with the empty-GUID sentinel `"00000000000000000000000000000000"` explicitly treated as a hard failure per R4.a). The EditMode test MUST NOT measure only in-memory state — it loads each asset via `AssetDatabase.LoadAssetAtPath<T>(path)`, invokes the delayCall-drain protocol from R4.a (`EditorApplication.delayCall` flush followed by explicit `AssetDatabase.SaveAssetIfDirty(asset)`), then re-imports via `AssetDatabase.ImportAsset(path, ImportAssetOptions.ForceUpdate)` before asserting, to guarantee any deferred `SetDirty` writes from R4 Persistence actually hit disk (`ImportAsset` alone does NOT drain `delayCall` queues, per R4.a). This test is wired into both (a) the standard CI EditMode suite and (b) an `IPreprocessBuildWithReport` hook with `callbackOrder = 100` (so Data Registry runs after package-provided preprocessors that register at the default `0`). The hook loads a known stale-`_id` fixture from `tests/editmode/data-registry/fixtures/stale-id/` and asserts the resulting `BuildReport.steps[]` contains an error message whose text includes the offending asset path — i.e., the hook must not merely throw or return false; it must emit a named, path-bearing diagnostic that surfaces in the build log. **BLOCKING CI**
...
- **AC-R5d — ToolConfig Upgrade-Cost Monotonicity** — GIVEN a `ToolConfig` fixture whose `upgradeTierChain` costs decrease at any step (e.g., `[10, 20, 15, 30]`), WHEN `OnValidate()` runs, THEN `Debug.LogWarning(message, this)` fires naming the offending tier index and an EditMode test asserts the warning is captured. A second fixture with strictly non-decreasing costs (`[10, 20, 20, 30]` — equal allowed) captures zero warnings. A third fixture with exactly two tiers `[5, 5]` (equal edge case) captures zero warnings. A parameterised EditMode test covers all three cases. This AC is BLOCKING at the Data Registry sprint — upgraded from pass-2 RECOMMENDED because the MVP shop reads `upgradeTierChain` directly (see Category #1), making a non-monotonic chain an immediately purchasable live gameplay bug rather than a future concern. **BLOCKING CI**
...
- **AC-R7a — Event ID Constant ↔ Taxonomy Drift** — GIVEN the full `src/` directory and the two Taxonomy SOs (`AudioEventTaxonomy`, `VisualEventTaxonomy`) loaded at test time, WHEN an EditMode test (`EventIdConstantDriftTest`) enumerates every `public const string` field whose declaring type's namespace matches `MoneyMiner.*` and whose value matches the taxonomy format regex, THEN every such constant value MUST exist as a registered `eventKey` in the matching Taxonomy SO (prefix `audio.` → AudioEventTaxonomy; prefix `vfx.` → VisualEventTaxonomy); unregistered constants fail the test with a named diagnostic listing the constant's file, class, field name, and the orphaned string. The reverse direction (taxonomy entries with no referring constant) is logged as an ADVISORY warning, not a failure — dead taxonomy entries are a housekeeping concern, not a Pillar-5 crack. This AC closes OQ-4's "hand-written drifts from taxonomy" failure mode without requiring a C# source generator at MVP; the source-generator decision can still land at architecture phase. **BLOCKING CI**
...
- **AC-R8d — Category-Wide Justification Modal (decision logic)** — GIVEN a synthetic `ToolCategory` fixture where 31% of tools have `_dominanceJustified = true` (e.g., 4 of 13), WHEN the method `BalanceAuditWindow.ShouldShowJustificationModal(category)` is called, THEN it returns `true`. A fixture at 30.7% (4 of 13 is 30.77%) returns `true`; a fixture at exactly 30.0% (3 of 10) returns `false`; a fixture with zero justified tools returns `false`. **Strict `>` comparator pinning**: add two explicit boundary-pin cases — (a) a fixture whose justification ratio computes to exactly `0.30f` (3 of 10 tools) MUST return `false`, and (b) a fixture at 31-of-100 tools (ratio = `0.31f`, a hair above threshold) MUST return `true`. Both pin the semantic that the threshold is strictly `>`, not `≥`, independent of float representation of the fraction. EditMode parameterised unit test covers the 30% boundary. Note: this AC covers the **decision logic only**; the modal's GUI rendering and sign-off mechanism are covered by AC-R8c (MANUAL). An EditMode test cannot observe an EditorWindow modal headlessly. **BLOCKING CI**

- **AC-R8e — Cross-Loadout Viability Playtest (Pillar 3 VS gate)** — GIVEN the VS playtest build with at least two complete tool loadouts defined in `production/playtests/loadout-fixtures/`, WHEN a playtest run exercises each loadout across ≥ 3 representative MVP levels, THEN **both gates must pass together**:
  1. **Floor gate**: each loadout completes (1-star or better) at least **60%** of the levels attempted (tunable — see Tuning Knobs).
  2. **Spread ceiling gate**: the 3-star pass rate of any single loadout may NOT exceed the 3-star pass rate of the worst loadout on the same level set by more than **40 percentage points** (tunable — see Tuning Knobs). A pair that 3-stars at 80% vs. 35% (45pp gap) FAILS this gate even if both clear the 60% floor, closing the "participation trophy" failure mode pass 3 flagged.

  The raw pass rates (both 1-star and 3-star, per loadout, per level) and qualitative observations are recorded in `production/qa/evidence/pillar3-viability-[YYYY-MM-DD].md`. This is the Pillar 3 validation gate that D.2's within-category audit cannot cover (see D.2 Scope Disclaim). Failure on either gate triggers a Tool System rebalance sprint before VS sign-off. **MANUAL** (playtest artifact gated at VS gate)
...
- **AC-EC-A2 — Asset Rename Preserves `_id`** — Two parts with independent gate levels:
  - **Part 1 (MANUAL evidence)**: GIVEN an SO asset with a known pre-rename `_id` value captured in the evidence log, WHEN the implementing engineer renames the asset in the Editor and separately drags it to a different folder, THEN `OnValidate` (rename case) and `OnPostprocessAllAssets.movedAssets[]` (folder-move case) each re-run, `_id` updates to the new GUID, and Inspector refresh confirms. Evidence artifact: `production/qa/evidence/data-registry-rename-move-[YYYY-MM-DD].md`, authored by the Data Registry implementation sprint's assigned engineer (owner role: unity-specialist or the sprint's assigned gameplay-programmer), containing the pre-/post-rename `_id` values plus screenshots of Inspector state before and after. The Data Registry sprint cannot be marked Done in the sprint board until this file exists with a date inside the sprint boundary. **MANUAL** (sign-off artifact).
  - **Part 2 (BLOCKING CI)**: An EditMode test pre-seeds a fixture with a known `_id`, renames it via `AssetDatabase.RenameAsset`, drains the R4.a delayCall protocol, and asserts the new `_id` matches the new path's GUID. A second test case moves the fixture via `AssetDatabase.MoveAsset`, drains, and asserts the same. Both failures are blocking. **BLOCKING CI**.

### H.4 Performance

- **AC-P1 — Catalog First-GetById Build Time (MVP scale)** — GIVEN any single-category Catalog with the MVP expected entry count (per category: Tools ≤ 20, Hazards ≤ 10, Loot ≤ 50, Levels ≤ 25, Biomes ≤ 3, others ≤ 30), WHEN the EditMode perf test triggers the Catalog's Dictionary build via the lazy-init path — i.e., calls `catalog.GetById(someKnownId)` on a freshly-instantiated Catalog instance whose internal Dictionary is still null (NOT via `OnEnable` alone, which under EC-R2's lazy-init pattern builds nothing) — THEN the measured wall-clock time for that first `GetById` call ≤ 4ms per Catalog (measured via `System.Diagnostics.Stopwatch`, reporting the median of 10 runs after 3 warmup iterations). **Noise-stability co-condition**: the sample's standard deviation must be `< 20%` of the median; if the first 10 runs fail this clamp, the test automatically extends up to 30 iterations and recomputes; if 30 iterations still fail the 20% stability clamp, the test FAILS with a diagnostic reporting median + stddev and is not silently passed. Rationale: a 4ms budget with ±2ms typical Editor noise is noise-gated at the timing layer — without a stddev clamp, a 3× real regression could still hit ≤ 4ms median by luck of scheduling variance. The 4ms-per-Catalog budget is chosen so the sequential sum across all 11 MVP Catalogs (11 × 4 = 44ms) fits within AC-P2a's 50ms Editor aggregate with 6ms headroom. The Dictionary is constructed with `new Dictionary<string, T>(entries.Count)` to pre-size capacity and avoid resize allocations. Note: EditMode timing runs in the Editor process and is typically 1.2–2.3× slower than IL2CPP Player-build timing on the same hardware; this budget is Editor-measured and is a proxy, not an authoritative on-device budget. On-device verification lives in AC-P2b. **ADVISORY**

- **AC-P2a — Catalog Load Time at Scene Entry (Editor aggregate)** — GIVEN all 11 Catalogs referenced by the scene's `DataRegistryBootstrapper` MonoBehaviour, WHEN the scene is entered in Editor PlayMode on developer hardware (cold start — first scene entry after Editor Play Mode initialization, not a warm scene transition), THEN total time from scene load to all Catalog first-access `GetById` returning valid results ≤ 50ms (median of 5 runs after 1 warmup). This budget is compositional with AC-P1: 11 Catalogs × 4ms per-Catalog = 44ms worst-case sequential, leaving 6ms for bootstrap overhead. AC-P2a is Editor-only and is the **Data Registry sprint close gate**; it is NOT a proxy for on-device performance — see AC-P2b. **ADVISORY** (Data Registry sprint gate)

- **AC-P2b — Catalog Load Time at Scene Entry (Mali-G52-class on-device)** — GIVEN the same scene and bootstrapper, WHEN the scene is entered on a Mali-G52-class Android device in an IL2CPP Player build (cold start), THEN total time ≤ 100ms (median of 5 runs after 1 warmup). AC-P2b is NOT a Data Registry sprint gate — it is a **Vertical Slice BLOCKING gate**, deadlined to the first Mali-G52-equipped playtest session of the VS phase. Rationale: (1) Mali-G52 hardware is not guaranteed available during the Data Registry sprint, so blocking sprint close on device measurements the team cannot take is impractical; (2) specialists disagree on the Editor-to-IL2CPP scaling factor (pessimistic 2.27× vs. realistic 1.2–1.5×) and renegotiating a single budget without device data risks shipping either a phantom-safe or phantom-failing number. The hard-deadline split approach (sprint ADVISORY here, VS BLOCKING later) is the adjudication of pass 3's specialist disagreement between performance-analyst B2 and creative-director. Before Data Registry sprint close, a sprint-backlog ticket tagged `hw-dependency: mali-g52` MUST be created in `production/sprints/` naming the specific VS playtest session that will verify AC-P2b; ticket-creation is itself a sprint DoD item (not "if available"). Evidence artifact on VS measurement: `production/qa/evidence/data-registry-p2b-mali-g52-[YYYY-MM-DD].md`. **PLAYMODE / BLOCKING at VS gate** (deferred from Data Registry sprint to VS)
...
- **AC-P4 — AssetPostprocessor (Layer 2) Latency** — GIVEN the v1.0-scale asset set (100+ levels, 50+ tools, all catalog categories populated), WHEN any single `OnPostprocessAllAssets` invocation fires (regardless of whether it carries 1 changed asset or 30 from a git-pull batch), THEN Layer 2 validation (duplicate-ID check, cross-catalog reference check, atlas discipline warning) completes ≤ 200ms per invocation (measured wall-clock via `Stopwatch` around the postprocessor entry point, median of 5 invocations). **Budget scope is per-invocation, not per-item**: a git pull that imports 30 assets produces 1–N postprocessor invocations depending on Unity's batching; each invocation independently must fit the 200ms budget. To make this achievable at batch scale, the postprocessor MUST scope all three check types to the delta — i.e., duplicate-ID check runs against `importedAssets[] ∪ movedAssets[]` cross-referenced against the existing Catalog index, NOT a full O(N²) re-scan of every registered entry — and atlas-discipline and cross-reference checks likewise scope to the changed set's referenced sprites/assets only. A full Catalog re-scan is explicitly out of the 200ms budget and must not be implemented inside the postprocessor hot path. This protects designer iteration velocity — violations block `save → iterate` workflow. If the budget is breached, designer authoring UX degrades and Addressables migration may be needed earlier than OQ-5 assumes. **ADVISORY**
...
- **AC-NHV — No Hardcoded Gameplay Constants in Source** — GIVEN the full `src/` directory, WHEN a CI grep job scans for named-constant assignments whose identifier matches the allowlist maintained at `tools/ci/nhv-allowlist.txt` (candidates: `_baseDamage`, `_dropWeight*`, `_currencyRate*`, `_quota*`, `_starThreshold*`, `_cooldown*`, `_arcMaxSeconds`), THEN zero matches appear outside files under `assets/data/` or files inheriting `DataRegistryEntry`. **Allowlist non-empty-bypass guard**: the CI job's first step, BEFORE any grep runs, asserts `wc -l < tools/ci/nhv-allowlist.txt` returns at least `7` (the MVP candidate count); if the allowlist file is missing, zero-byte, or has fewer than 7 non-comment lines, the CI job FAILS with a named diagnostic "NHV allowlist degraded — refusing to silently pass." This closes the empty-bypass failure mode where deleting the allowlist produces zero grep matches (trivially "passing" the AC) while actually disabling the entire hardcoded-constant check. The allowlist is human-maintained and reviewed at every sprint; new gameplay-constant identifiers must be added before they can be referenced; reductions below the MVP floor of 7 require an explicit ADR entry justifying the removal. The grep does NOT match generic integer or float literals (which produce unacceptably high false-positive rates) — it only matches allowlisted identifier names. Blocking failure produces a list of offending file paths and line numbers. **BLOCKING CI**

### Gate Summary (after pass-3 revision)

| Gate Level | Count | AC Identifiers |
|---|---|---|
| **BLOCKING CI** (Data Registry sprint gates) | 22 | R1a, R1b, R2, R3, R4, R5a, R5b, R5c, R5d (new — monotonic upgrade cost), R7, R7a (new — event-ID drift), R8a, R8b, R8d (decision logic), D1, D2, D3, EC-A1, EC-A2 Part 2, EC-B2, EC-R1, AC-NHV |
| **ADVISORY** (Data Registry sprint gates) | 6 | R6 atlas warning, R8 balance audit count, P1 first-GetById build time, P2a scene-load Editor, P3 memory schema, P4 Layer 2 latency |
...
| **BLOCKING at VS gate** (deferred from Data Registry sprint) | 2 | P2b on-device Mali-G52 load time, R8e cross-loadout Pillar 3 playtest |
| **MANUAL** (Data Registry sprint sign-off artifacts) | 2 | R8c balance audit window sign-off, EC-A2 Part 1 rename/move evidence |
| **(Covered — no separate gate)** | 1 | EC-F1 — fixture row inside AC-D3's parameterized test; not a standalone CI gate |

**Total**: **42 AC identifiers** tracked in this document (22 BLOCKING CI + 6 ADVISORY + 11 DEFERRED + 2 VS-BLOCKING + 2 MANUAL = 43; minus 1 fixture-only identifier EC-F1 that produces no independent CI test file, and minus the AC-EC-A2 double-count — it is listed once as "BLOCKING CI (Part 2)" and once as "MANUAL (Part 1)" = 2 rows for one AC, so subtract 1 → **42 distinct AC identifiers + 1 covered-in-D3 row**). The Data Registry implementation sprint is gated on: **22 BLOCKING CI + 6 ADVISORY + 2 MANUAL + 1 MANUAL (AC-R8c) = 30 sprint-close gates**. The two VS-BLOCKING ACs (P2b, R8e) do not block Data Registry sprint close; they are tracked as deferred VS deadlines with backlog tickets. **Scaffolding count**: **26 distinct test files or sign-off artifacts authored during the Data Registry sprint** — 22 BLOCKING CI test files (EC-A2 Part 2 shares a test file with AC-R4's GUID-consistency suite, so counts as 1 scaffolding unit) + 2 MANUAL sign-off artifact files (R8c + EC-A2 Part 1) + 1 implicit scaffolding unit for AC-NHV's allowlist file + 1 for the stale-id fixture required by AC-R4's IPreprocessBuildWithReport = **26 scaffolding units**. The 11 DEFERRED PLAYMODE ACs and 2 VS-BLOCKING ACs are scaffolded by their receiving sprints.

**Shift-left note for implementation**: the 22 BLOCKING CI criteria should have stub EditMode / Roslyn-analyzer test files created at story start, before any SO schema is authored. The PLAYMODE integration criteria require a test scene fixture with pre-populated Catalog assets — this fixture is scaffolded during the same sprint as Catalog implementation, not deferred to QA hand-off. The Layer 2 fixture set at `tests/editmode/data-registry/fixtures/layer2/` (required by AC-R5b), the hex-edit fixture at `tests/editmode/data-registry/fixtures/hex-edit/` (required by AC-EC-B2), and the stale-id fixture at `tests/editmode/data-registry/fixtures/stale-id/` (required by AC-R4 IPreprocessBuildWithReport) are authored in the sprint's first story, before any formula tests land. The `tools/ci/nhv-allowlist.txt` file is initialized with the 7 MVP candidate identifiers in the same first story so AC-NHV's non-empty-bypass guard passes from day one.
